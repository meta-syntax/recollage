...
| `lastViewedAt` が永遠に null（旧 #4） | 詳細ページ＋recordView 実装（同日）。モック段階は localStorage オーバーレイ。フェーズ4で Supabase の UPDATE に差し替え |
...
| 9 | カテゴリのスナップショット方式 | 初回変更時に `recollage:categories` へ全量複製し、以降 JSON 側（再インポート）のカテゴリ追加が反映されない。エントリ側（created/edits のパッチ方式）との非対称 | フェーズ4（Supabase 移行でオーバーレイ自体が消える） |
| 10 | 書き込み系が localStorage 先行 | createEntry/updateEntry/カテゴリCRUD はモック実装（ADR-011 実装時）。roadmap の「書き込み系は Supabase とセット」を、検証期間中にキャプチャ可能にする目的で前倒し。データはブラウザローカルにのみ存在し、消えると失われる | フェーズ4（Supabase 移行時に localStorage 分のマイグレーションを書く） |
