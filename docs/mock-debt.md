...
| # | 項目 | 内容 | 払うタイミング |
|---|---|---|---|
| 1 | index.vue モノリス | 536行に レイアウト＋カード3種＋scoped CSS 347行が同居。ADR-005 の2層コンポーネント（feature/ui）未分割 | フェーズ4入り口。入力フォーム等で画面が増える前 |
| 2 | 生 hex 直書き | ADR-007 の Primitive/Semantic トークン・`@theme` 未導入。Claude Design からの「値の逆抽出」が前提タスク（open-questions「あえて先送り」） | トークン逆抽出の実施時にまとめて |
